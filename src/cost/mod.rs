...
pub trait Cost<D: Dimension> {
    fn loss(&self, predicted: &ArrayBase<OwnedRepr<f32>, D>, target: &ArrayBase<OwnedRepr<f32>, D>) -> f32;
    fn gradient(&self, predicted: &ArrayBase<OwnedRepr<f32>, D>, target: &ArrayBase<OwnedRepr<f32>, D>) -> ArrayBase<OwnedRepr<f32>, D>;
}
...
impl<D: Dimension> Cost<D> for CrossEntropy {
    fn loss(&self, predicted: &ArrayBase<OwnedRepr<f32>, D>, target: &ArrayBase<OwnedRepr<f32>, D>) -> f32 {
        ...
        -(target * &predicted_clipped.mapv(|v| v.ln())).mean().unwrap()
    }
