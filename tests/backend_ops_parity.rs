...
fn range_4d(shape: (usize, usize, usize, usize)) -> Array<f32, ndarray::Ix4> {
    let n = shape.0 * shape.1 * shape.2 * shape.3;
    Array::from_shape_vec(shape, (0..n).map(|v| v as f32 / n as f32 - 0.25).collect()).unwrap()
}
...
#[test]
fn unary_domain_ops_parity() {
    let x = arr2(&[[0.25_f32, 1.0, 2.25], [4.0, 9.0, 16.0]]);
    let gx = GPUBackend::from_array(x.clone());

    let ln_cpu = CPUBackend::unary(&x, unary_ops::LN);
    let ln_gpu = GPUBackend::to_array(&GPUBackend::unary(&gx, unary_ops::LN));
    assert_all_finite("ln", &ln_gpu);
    assert_close("ln", &ln_cpu, &ln_gpu, 1e-6);

    let sqrt_cpu = CPUBackend::unary(&x, unary_ops::SQRT);
    let sqrt_gpu = GPUBackend::to_array(&GPUBackend::unary(&gx, unary_ops::SQRT));
    assert_all_finite("sqrt", &sqrt_gpu);
    assert_close("sqrt", &sqrt_cpu, &sqrt_gpu, 1e-6);
}
...
fn scalar_parity() {
    ...
    check!(
        "scale",
        CPUBackend::scale(&x, 2.5),
        GPUBackend::scale(&gx, 2.5),
        1e-6
    );
    check!(
        "scale_neg",
        CPUBackend::scale(&x, -0.5),
        GPUBackend::scale(&gx, -0.5),
        1e-6
    );
    check!(
        "scale_zero",
        CPUBackend::scale(&x, 0.0),
        GPUBackend::scale(&gx, 0.0),
        1e-6
    );
    check!(
        "scalar_sub",
        CPUBackend::scalar_sub(1.25, &x),
        GPUBackend::scalar_sub(1.25, &gx),
        1e-6
    );
    check!(
        "scalar_max",
        CPUBackend::scalar_max(&x, 0.25),
        GPUBackend::scalar_max(&gx, 0.25),
        1e-6
    );
    check!(
        "scalar_min",
        CPUBackend::scalar_min(&x, 0.25),
        GPUBackend::scalar_min(&gx, 0.25),
        1e-6
    );
    check!(
        "clamp",
        CPUBackend::clamp(&x, -0.3, 1.0),
        GPUBackend::clamp(&gx, -0.3, 1.0),
        1e-6
    );
}
...
fn binop_parity() {
    ...
    assert_close(
        "add",
        &CPUBackend::add(&a, &b),
        &GPUBackend::to_array(&GPUBackend::add(&ga, &gb)),
        1e-6,
    );
    assert_close(
        "sub",
        &CPUBackend::sub(&a, &b),
        &GPUBackend::to_array(&GPUBackend::sub(&ga, &gb)),
        1e-6,
    );
    assert_close(
        "mul",
        &CPUBackend::mul(&a, &b),
        &GPUBackend::to_array(&GPUBackend::mul(&ga, &gb)),
        1e-6,
    );
    assert_close(
        "div",
        &CPUBackend::div(&a, &b),
        &GPUBackend::to_array(&GPUBackend::div(&ga, &gb)),
        1e-6,
    );
}
...
fn matmul_2d_parity() {
    ...
    assert_close(
        ...
        &GPUBackend::to_array(&GPUBackend::matmul(
            &GPUBackend::from_array(a),
            &GPUBackend::from_array(b),
        )),
        1e-4,
    );
}
...
fn matmul_4d_parity() {
    let a = Array::from_shape_vec(
        (2, 2, 3, 4),
        (0..48).map(|v| v as f32 / 9.0 - 2.0).collect(),
    )
    .unwrap();
    let b =
        Array::from_shape_vec((4, 5), (0..20).map(|v| v as f32 / 11.0 - 0.3).collect()).unwrap();
    assert_close(
        "matmul_4d",
        &CPUBackend::matmul(&a, &b),
        &GPUBackend::to_array(&GPUBackend::matmul(
...
#[test]
fn matmul_3d_batched_rhs_parity() {
    let a = range_3d((2, 3, 4));
    let b = Array::from_shape_vec((2, 4, 5), (0..40).map(|v| v as f32 / 13.0 - 0.75).collect())
        .unwrap();
    assert_close(
        "matmul_3d_batched_rhs",
        &CPUBackend::matmul(&a, &b),
        &GPUBackend::to_array(&GPUBackend::matmul(
            &GPUBackend::from_array(a),
            &GPUBackend::from_array(b),
...
#[test]
fn matmul_4d_batched_rhs_parity() {
    let a = range_4d((2, 2, 3, 4));
    let b = Array::from_shape_vec(
        (2, 2, 4, 5),
        (0..80).map(|v| v as f32 / 17.0 - 1.0).collect(),
    )
    .unwrap();
    assert_close(
        "matmul_4d_batched_rhs",
        &CPUBackend::matmul(&a, &b),
        &GPUBackend::to_array(&GPUBackend::matmul(
            &GPUBackend::from_array(a),
            &GPUBackend::from_array(b),
...
fn broadcast_add_parity() {
    ...
    assert_close(
        ...
        &GPUBackend::to_array(&GPUBackend::broadcast_add(
            &GPUBackend::from_array(a),
            &GPUBackend::from_array(b),
        )),
        1e-6,
...
#[test]
fn broadcast_add_3d_parity() {
    let a = range_3d((2, 3, 4));
    let b = arr1(&[0.5_f32, -1.0, 2.0, -0.25]);
    assert_close(
        "broadcast_add_3d",
        &CPUBackend::broadcast_add(&a, &b),
        &GPUBackend::to_array(&GPUBackend::broadcast_add(
...
fn softmax_vjp_parity() {
    ...
    let gpu = GPUBackend::to_array(&GPUBackend::softmax_vjp(
        &GPUBackend::from_array(z),
        &GPUBackend::from_array(grad),
    ));
...
#[test]
fn sum_axis_4d_parity() {
    let x = range_4d((2, 2, 3, 4));
    let gx = GPUBackend::from_array(x.clone());
    for axis in 0..4 {
        assert_close(
            &format!("sum_axis_4d_{axis}"),
            &CPUBackend::sum_axis(&x, axis),
            &GPUBackend::to_array(&GPUBackend::sum_axis(&gx, axis)),
            1e-5,
        );
...
#[test]
fn select_axis1_parity() {
    let x = Array::from_shape_vec((2, 5, 3), (0..30).map(|v| v as f32).collect()).unwrap();
    let gx = GPUBackend::from_array(x.clone());
    let idx = [4usize, 2, 0];
    assert_close(
        "select_axis1",
        &CPUBackend::select(&x, 1, &idx),
        &GPUBackend::to_array(&GPUBackend::select(&gx, 1, &idx)),
        1e-6,
    );
}
